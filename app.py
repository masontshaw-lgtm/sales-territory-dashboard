...
OVERDUE_LABEL = "Overdue — follow up"
...
def add_flags(frame, report_date):
    ...
    frame.loc[active & frame.next_follow_up.lt(today), "follow_up_flag"] = OVERDUE_LABEL
...
def main():
    ...
    if follow_only:
        view = view[view.follow_up_flag.isin([OVERDUE_LABEL, "Due today", "Missing date"])]
    ...
    st.write(f'Follow-up: {flags.get(OVERDUE_LABEL, 0)} overdue - follow up · {flags.get("Due today", 0)} due today · '
    ...
    due = view[view.follow_up_flag.isin([OVERDUE_LABEL, "Due today", "Missing date"])].sort_values("next_follow_up", na_position="last")
    if due.empty:
